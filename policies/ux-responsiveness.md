...
- The terminal-editor handoff (`e`) blocks the event loop for the whole editor session. This is a deliberate, permanent exception: the editor owns the pane, and reviewr can neither draw nor read input while it does (`specs/input.md` Edit). A window editor is spawned and never waited on, so the pane keeps answering, and its write lands on the ordinary poll like any other change to the worktree — reviewr adds no watch, wake, or timer for it.
