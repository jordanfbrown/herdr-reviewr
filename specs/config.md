...
| `editor`             | the editor command, `{file}` and `{line}` substituted (`input.md`)                 |
...
`editor` is the whole command line, split on whitespace with double or single quotes holding a word together, and no shell runs it. `$VISUAL` and `$EDITOR` split the same way, so a path containing a space is spelled the same in either place. `{file}` and `{line}` are the whole grammar. reviewr substitutes each with the path and the line number, as many times as it appears, in one pass, so a path that spells `{line}` is a file name and never a second placeholder. Any other `{` is an invalid value (→ CFG-WHOLE-FILE), one that closes nothing included, since it would otherwise reach the editor as a literal word. A value that does not name `{file}` gets the path appended. The key sets the command outright, so reviewr adds no flag of its own to it. Which editors keep the pane is `input.md` Edit's. Unset, the editor is `$VISUAL`, else `$EDITOR`, and its own name picks the arguments (`input.md` Edit). Neither set is the no-editor outcome, never an invalid config. A value that survives validation but names no program, `""` spelled as two quote characters, opens nothing and says which of the two it is. An empty or non-string value is invalid (→ CFG-WHOLE-FILE).
