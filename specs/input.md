...
- Failing that, the binary's own name answers which kind it is, in every path, the `editor` key's command included: an editor needs a wait flag exactly when it hands the file to a window and returns. A name reviewr does not know is handed the pane, which is the guess a terminal editor can survive.
