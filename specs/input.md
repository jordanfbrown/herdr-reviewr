...
- A press whose editor name resolves to no program on the reviewer's `PATH` says so and opens nothing, before the pane changes hands. A handover for a spawn that cannot happen would flash the shell on every press.
...
- A window editor never reads the terminal, so it keeps none of it. The press reports `opened <path>` and nothing else moves. reviewr never waits on one and never watches one between presses. It asks a launched editor only whether it has already exited, at the next press, so a session leaves no children behind. The reviewer keeps the diff on screen for the whole edit, and the write shows on the next poll like any other change to the worktree (`tui.md`). Pressing the key again on a file already out opens it again, at the line now under the cursor.
...
- A window editor that fails to launch reports on the pane, since its own output goes nowhere. A failure after that is the editor's own to report, in its own window.
